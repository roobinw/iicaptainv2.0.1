...
interface EventCardBaseProps {
  item: Match | Training;
  eventType: "match" | "training" | "refereeing"; // Added "refereeing"
  icon: ReactNode;
  titlePrefix?: string;
...
export function EventCardBase({
  ...
  useEffect(() => {
    if (currentUser?.teamId && eventType !== "refereeing") { // Refereeing assignments don't have attendance
  ...
  }, [item.attendance, memberList, initializeAttendance, eventType]);
      ...
      <CardFooter className="border-t pt-4 flex justify-between items-center">
        {eventType !== "refereeing" ? (
            ...
            <DialogTrigger asChild>
                <Button variant="outline" className="w-auto hover:bg-accent hover:text-accent-foreground">
            ...
            <div /> // Empty div to maintain space if no attendance button
        )}
