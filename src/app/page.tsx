...
// Metadata for this page is defined in src/app/layout.tsx
...
export default function LandingPage() {
  ...
  useEffect(() => {
    if (!isLoading) {
      if (user && user.teamId) {
        ...
        // If user exists but no teamId, they might be in onboarding flow
        // or need to create/join a team.
        // Redirect to create-team if they landed here without a teamId.
        if (router.pathname !== "/onboarding/create-team" && router.pathname !== "/login" && router.pathname !== "/signup") {
             router.replace("/onboarding/create-team");
        }
      }
      // If no user and not loading, they stay on the landing page.
    }
  ...
  if (isLoading || (!isLoading && user && user.teamId)) { // Show loading if auth is processing OR if user has a teamId (about to be redirected)
    return (
  ...
  // If !isLoading and (!user or (user and !user.teamId)), render the landing page.
  return (
            ...
            >
             ...
             >
                <Image
                    ...
                    priority
                    onError={(e) => (e.currentTarget.src = 'https://placehold.co/1200x600.png')}
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                { icon: Icons.Users, title: "Player Roster Management", description: "Maintain a comprehensive list of your team members with roles, contact details, and performance notes. All in one secure place." },
