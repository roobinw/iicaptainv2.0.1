...
*   **Subcollection of `teams/{teamId}`: `messages`** (NEW)
...
**Example `messages` subcollection document (`/teams/uniqueTeamId123/messages/messageIdAbc`):** (NEW)
```json
{
    ...
    **Security Rules:**
    ...
    service cloud.firestore {
      match /databases/{database}/documents {
        ...
        // Messages subcollection (nested under a specific team) (NEW)
        match /teams/{teamId}/messages/{messageId} {
          ...
          // Future: Update might be allowed for pinning by admin, or editing by original author (admin).
          // For now, keeping update false or restricted.
